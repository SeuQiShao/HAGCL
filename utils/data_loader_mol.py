...
class MoleculeDataset_aug_rgcl(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        if self.aug == 'dropN':
            nodes_score = self.node_score[self.slices['x'][idx]:self.slices['x'][idx + 1]]
            data = hete_nodes_prob(data, self.aug_ratio, nodes_score)
        elif self.aug == 'dropN_cp':
            nodes_score = self.node_score[self.slices['x'][idx]:self.slices['x'][idx + 1]]
            data = hete_nodes_prob(data, self.aug_ratio, nodes_score, cp = True)
...
def hete_nodes_prob(data, rate, node_score, cp = False):
...
def meta_data(node_key, edge_key):
    node, edge = [], []
    for i in node_key:
        node.append(i)
